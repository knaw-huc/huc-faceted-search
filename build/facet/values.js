...
class FacetValues extends React.PureComponent {
    ...
    render() {
        return (React.createElement(Wrapper, { innerRef: this.wrapperRef },
            React.createElement(List, null, this.state.values.map(value => React.createElement(value_1.default, { addFilter: () => this.props.state.requestManager.addFilter(this.props.field, value.key), key: value.key, removeFilter: () => this.props.state.requestManager.removeFilter(this.props.field, value.key), value: value }))),
            React.createElement(button_1.MoreLessButton, { onClick: () => this.props.state.requestManager.viewMoreFacetValues(this.props.id, this.props.field, this.props.size) }, "View more"),
            this.state.values.length && this.props.size !== this.state.values.length &&
                React.createElement(button_1.MoreLessButton, { onClick: () => this.props.state.requestManager.viewLessFacetValues(this.props.id, this.props.field, this.props.size) }, "View less")));
    }
